...
brt_function <- function(data, extrapolation_data, out_dir, no_iter, split_ratio, add_subregion, limited_data_test){
  library(enmSdmX)
  ...
  for(i in final.folds){
    ...
    if(class(extrapolation_data)=="data.frame"){
      extrapolation_data <- extrapolation_data[complete.cases(extrapolation_data),]#; extrapolation_data$presence <- as.factor(extrapolation_data$presence)
    }
    ...
    if(add_subregion==TRUE){ #if TRUE/YES, leaves subregion column in as a predictor
      sdm_brt <- trainBRT(data = train_complete, 
                          resp = 'presence', 
                          preds = names(subset(train_complete, select=-c(presence))),#take out response from test data
                          w = FALSE,
                          out = "model",
                          )
      ...
      pred <- as.data.frame(predict(sdm_brt, newdata=subset(test_complete, select=-c(presence)), type = 'response')) #take out response from test data
      ...
      pred <- as.data.frame(predict(sdm_brt, newdata=subset(test_complete, select=-c(presence,subregion)), type = 'response')) #take out response from test data
      ...
      #       store separately for own set of performance measures
      if(class(extrapolation_data)=="data.frame"){
        pred.extrapolate <- as.data.frame(predict(sdm_brt, newdata=subset(extrapolation_data, select=-c(presence)), type = 'response')) #take out response and ask model to predict outside of subregion
      }
    }
    ...
    if(1 %in% test_complete$presence && 0 %in% test_complete$presence) {
      ...
      print(auc$auc)
      #pROC::plot.roc(roc(response=as.numeric(test_complete[,1]), predictor=as.numeric(pred[,1])))
      ## Add pROC
    ...
    if(class(extrapolation_data)=="data.frame"){
      ...
      best.threshold.extrapolate <- pROC::coords(auc.extrapolate, "best", ret = "threshold") #find own best threshold (not necessarily same as above)
    ...
    if("subregion" %in% colnames(data)){
      ...
      for (k in 1:length(unique(data$subregion))) { #loop through each subregion
        ...
        # Create storage dataframe to get overwritten during each iteration of the loop
        ...
        if (dim(test_subregion)[1] != 0) {
          pred.subregion <- as.data.frame(predict(sdm_brt, newdata=subset(test_subregion, select=-c(presence)), type = 'response')) #take out response from test data
          # Store predictions for ensemble model evaluation
    ...
    #variable importance
    #imp_df <- as.data.frame(sdm_brt$contributions); imp_df$var <- rownames(imp_df) #get variable names for pdp loop below
    imp_df <- as.data.frame(vi_shap(sdm_brt, feature_names = names(subset(train_complete, select=-c(presence))), train = train_complete, pred_wrapper = predict))
    ...
    for (j in 1:nrow(imp_df)) { #loop through each variable
      ...
      output <- as.data.frame(pdp::partial(sdm_brt, pred.var = imp_df[j,1], prob = TRUE, train = train_complete, n.trees=sdm_brt$n.trees))
  ...
  metrics.output.dataframe <- as.data.frame(cbind(auc_list,  pauc_mean_list, pauc_ratio_list, pauc_pval_list, fscore_list, sensitivity_list, specificity_list, tss_list)); colnames(metrics.output.dataframe) <- c("auc", "pauc_mean", "pauc_ratio", "pauc_pval",
                                                                                                                                                                                                                   "fscore", "sensitivity", "specificity", "tss")
  write.csv(metrics.output.dataframe, paste0(out_dir, "/brt_baseline_metrics.csv"))
  write.csv(oob_list, paste0(out_dir, "/brt_oob.csv"))
  ...
  if(class(extrapolation_data)=="data.frame"){
    ...
    write.csv(extrapolate.metrics.output.dataframe, paste0(out_dir, "/brt_extrapolate_metrics.csv"))
  }
  
  if("subregion" %in% colnames(data)) {
    write.csv(metrics_subregion_df_iter, paste0(out_dir, "/brt_subregion_metrics.csv"))
    write.csv(pred_storage_subregion_iter, paste0(out_dir, "/brt_subregion_preds.csv"))
  }
  
  write.csv(imp_df_iter, paste0(out_dir, "/brt_imp_df.csv")) 
  write.csv(pd_df_iter, paste0(out_dir,"/brt_pdp.csv"))
  write.csv(pred_storage_iter, paste0(out_dir,"/brt_preds.csv")) 
}

# use function!
#reminder of function call & inputs
#brt_function(data, extrapolation_data, out_dir, no_iter, split_ratio, add_subregion, limited_data_test)
all_data <- readRDS("~/Desktop/late_feb_runs/datasets/glabrata_national.rds")
brt_function(all_data, NA, "~/Desktop/late_feb_runs/glabrata_mg_models/", 10, 0.8, FALSE, FALSE)
...
for (i in file_incides) {
  ...
  brt_function(all_data, NA, paste("~/Desktop/r&r_model_runs/", output_folder_name, sep=""), 10, 0.8, FALSE, FALSE)
  print(paste("File", i, "of", length(files), "done", sep =" "))
}

# brt.change <- read.csv("~/Desktop/early_dec_runs/straminea_mg_models/brt_baseline_metrics.csv")
# brt.change <- brt.change[,-c(1,2)]
# brt.change
# brt.change$auc[1] <- 0.45
# brt.change
# write.csv(brt.change,"~/Desktop/early_dec_runs/straminea_mg_models/brt_baseline_metrics.csv")

# Function use instructions and examples
# (1) Simple
#     Ex. brt_function(all_data, NA, "~/Desktop/output_data/straminea_national_models/", 25, 0.8, FALSE, FALSE)
...
#     Ex. brt_function(all_data, extrapolation_data, "~/Desktop/output_data/straminea_national_models_w_extrapolation/", 25, 0.8, FALSE, FALSE)
...
#     Ex. brt_function(all_data, NA, "~/Desktop/output_data/straminea_national_models_w_subregion/", 25, 0.8, TRUE, FALSE)
# (4) Limited data test
...
#     Ex. brt_function(all_data, NA, "~/Desktop/output_data/straminea_national_models_limited_data/", 25, 0.8, FALSE, TRUE)
