...
fig2a <- plot(me_pred_raster,# main="A",  
              ...
              breaks = c(0,seq(0.025, me_pred_raster@data@max, (me_pred_raster@data@max-0.02)/7)), col = colors2,
...
stram_auc_df_summary <- stram_auc_df %>% 
  ...
  summarise(
    ...
    sd.high = mean(auc)+(sd(auc)/sqrt(n)) ,
    sd.low = mean(auc)-(sd(auc)/sqrt(n)))
...
tenag_auc_df_summary <- tenag_auc_df %>% 
  ...
  summarise(
    ...
    second.min = second.smallest.func(auc),
    second.max = second.largest.func(auc),
    sd.high = mean(auc,na.rm=T)+(sd(auc,na.rm=T)/sqrt(n)) ,
    sd.low = mean(auc,na.rm=T)-(sd(auc,na.rm=T)/sqrt(n)))
...
stram_pauc_pval_df_summary <- stram_pauc_pval_df %>% 
  ...
  summarise(n = n(), 
            ...
            sd.high = mean(pauc_pval, na.rm=T)+(sd(pauc_pval, na.rm=T)/sqrt(n)) ,
            sd.low = mean(pauc_pval, na.rm=T)-(sd(pauc_pval, na.rm=T)/sqrt(n)))
...
tenag_mg_models_pauc_pval_df$geo_extent <- "São Paulo"
...
glab_pauc_pval_df_summary[c(5,6,4),c(1,2,3,6,7)]
stram_pauc_pval_df_summary[c(5,6,4),c(1,2,3,6,7)]
tenag_pauc_pval_df_summary[c(2,3,1),c(1,2,3,6,7)]
```
...
correlation_function <- function(rasters_dir1, rasters_dir2, model_type){
  ...
  for(ii in 1:100){
    ...
    state_raster_masked <- mask(x = state_raster, mask = mg_shapefile) #change shapefile!
    state_raster_cropped <- crop(state_raster_masked, mg_shapefile)
  ...
  write.csv(cor_df, paste(out_name,"stram_",model_type,"_corr_permutations.csv",sep=""))
  #write.csv(cor_pval_df, paste(out_name,"stram_",model_type,"_corr_pvals.csv",sep=""))
}
model_types <- c("rf")
for(k in 1:length(model_types)){
  correlation_function("~/Desktop/r&r_predictions/straminea_national_models/", "~/Desktop/r&r_predictions/straminea_mg_models/",model_types[k])
}

glab_me_corr <- as.data.frame(read.csv("~/Desktop/r&r_figures/table4/glabrata_maxent_corr.csv"))
mean(glab_me_corr[,2])
sd.error.high(glab_me_corr[,2])
sd.error.low(glab_me_corr[,2])

glab_rf_corr <- as.data.frame(read.csv("~/Desktop/r&r_figures/table4/glabrata_rf_corr.csv"))
mean(glab_rf_corr[,2])
sd.error.high(glab_rf_corr[,2])
sd.error.low(glab_rf_corr[,2])

glab_brt_corr <- as.data.frame(read.csv("~/Desktop/r&r_figures/table4/glabrata_brt_corr.csv"))
mean(glab_brt_corr[2:10,2])
sd.error.high(glab_brt_corr[,2])
sd.error.low(glab_brt_corr[,2])

stram_me_corr <- as.data.frame(read.csv("~/Desktop/r&r_figures/table4/straminea_maxent_corr.csv"))
mean(stram_me_corr[,2])
sd.error.high(stram_me_corr[,2])
sd.error.low(stram_me_corr[,2])

stram_rf_corr <- as.data.frame(read.csv("~/Desktop/r&r_figures/table4/straminea_rf_corr.csv"))
mean(stram_rf_corr[,2])
sd.error.high(stram_rf_corr[,2])
sd.error.low(stram_rf_corr[,2])

stram_brt_corr <- as.data.frame(read.csv("~/Desktop/r&r_figures/table4/straminea_brt_corr.csv"))
mean(stram_brt_corr[,2])
sd.error.high(stram_brt_corr[,2])
sd.error.low(stram_brt_corr[,2])

tenag_me_corr <- as.data.frame(read.csv("~/Desktop/r&r_figures/table4/tenagophila_maxent_corr.csv"))
mean(tenag_me_corr[,2])
sd.error.high(tenag_me_corr[,2])
sd.error.low(tenag_me_corr[,2])

tenag_rf_corr <- as.data.frame(read.csv("~/Desktop/r&r_figures/table4/tenagophila_rf_corr.csv"))
mean(tenag_rf_corr[,2])
sd.error.high(tenag_rf_corr[,2])
sd.error.low(tenag_rf_corr[,2])

tenag_brt_corr <- as.data.frame(read.csv("~/Desktop/r&r_figures/table4/tenagophila_brt_corr.csv"))
mean(tenag_brt_corr[,2])
sd.error.high(tenag_brt_corr[,2])
sd.error.low(tenag_brt_corr[,2])
